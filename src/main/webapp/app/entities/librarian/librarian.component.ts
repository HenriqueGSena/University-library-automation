import { Component, Vue, Inject } from 'vue-property-decorator';
import Vue2Filters from 'vue2-filters';
import { ILibrarian } from '@/shared/model/librarian.model';

import LibrarianService from './librarian.service';
import AlertService from '@/shared/alert/alert.service';

@Component({
  mixins: [Vue2Filters.mixin],
})
export default class Librarian extends Vue {
  @Inject('librarianService') private librarianService: () => LibrarianService;
  @Inject('alertService') private alertService: () => AlertService;

  private removeId: number = null;

  public librarians: ILibrarian[] = [];

  public isFetching = false;

  public mounted(): void {
    this.retrieveAllLibrarians();
  }

  public clear(): void {
    this.retrieveAllLibrarians();
  }

  public retrieveAllLibrarians(): void {
    this.isFetching = true;
    this.librarianService()
      .retrieve()
      .then(
        res => {
          this.librarians = res.data;
          this.isFetching = false;
        },
        err => {
          this.isFetching = false;
          this.alertService().showHttpError(this, err.response);
        }
      );
  }

  public handleSyncList(): void {
    this.clear();
  }

  public prepareRemove(instance: ILibrarian): void {
    this.removeId = instance.id;
    if (<any>this.$refs.removeEntity) {
      (<any>this.$refs.removeEntity).show();
    }
  }

  public removeLibrarian(): void {
    this.librarianService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A Librarian is deleted with identifier ' + this.removeId;
        this.$bvToast.toast(message.toString(), {
          toaster: 'b-toaster-top-center',
          title: 'Info',
          variant: 'danger',
          solid: true,
          autoHideDelay: 5000,
        });
        this.removeId = null;
        this.retrieveAllLibrarians();
        this.closeDialog();
      })
      .catch(error => {
        this.alertService().showHttpError(this, error.response);
      });
  }

  public closeDialog(): void {
    (<any>this.$refs.removeEntity).hide();
  }
}
