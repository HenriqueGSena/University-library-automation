...
/**
 * REST controller for managing {@link br.com.universityautomation.developer.domain.Registration}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Registration> partialUpdateRegistration(
        ...
        Optional<Registration> result = registrationRepository
            ...
            .map(existingRegistration -> {
                if (registration.getMatricula() != null) {
                    existingRegistration.setMatricula(registration.getMatricula());
                }
                if (registration.getNome() != null) {
                    existingRegistration.setNome(registration.getNome());
                }
                if (registration.getDataNascimento() != null) {
                    existingRegistration.setDataNascimento(registration.getDataNascimento());
                }
                if (registration.getEndereco() != null) {
                    existingRegistration.setEndereco(registration.getEndereco());
                }
                if (registration.getTelefone() != null) {
                    existingRegistration.setTelefone(registration.getTelefone());
                }
