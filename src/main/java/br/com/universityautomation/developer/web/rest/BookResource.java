...
/**
 * REST controller for managing {@link br.com.universityautomation.developer.domain.Book}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Book> partialUpdateBook(@PathVariable(value = "id", required = false) final Long id, @RequestBody Book book)
        ...
        Optional<Book> result = bookRepository
            ...
            .map(existingBook -> {
                ...
                if (book.getEditora() != null) {
                    existingBook.setEditora(book.getEditora());
                }
                if (book.getEdicao() != null) {
                    existingBook.setEdicao(book.getEdicao());
                }
